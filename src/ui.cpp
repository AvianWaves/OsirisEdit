...
static void menuKeyCommands()
{
	...
	if (ImGui::IsKeyPressed(ImGuiKey_F1, ImGuiInputFlags_Repeat))
		menuManual();
	...
	if (!io.KeySuper && !io.KeyCtrl && !io.KeyShift && !io.KeyAlt)
	{
		...
		if (!g.ActiveId || g.ActiveId != GImGui->InputTextState.ID)
		{
			...
			/*if (ImGui::IsKeyPressed(ImGuiKey_6))
				currentPage = DB_PAGE;*/
			if (ImGui::IsKeyPressed(ImGuiKey_UpArrow, ImGuiInputFlags_Repeat))
			...
			if (ImGui::IsKeyPressed(ImGuiKey_RightArrow, ImGuiInputFlags_Repeat))
				incrementSelectedId(1);
		}
	}
}
...
void renderWaveMenu()
{
	...
	{
		if (ImGui::MenuItem("Copy All", ImGui::GetIO().ConfigMacOSXBehaviors ? "Cmd+C" : "Ctrl+C", false, selectedId - lastSelectedId != 0))
		{
			menuCopyAll();
		}
	}
	if (ImGui::MenuItem("Paste All", ImGui::GetIO().ConfigMacOSXBehaviors ? "Cmd+V" : "Ctrl+V", false, clipboardArrayActive))
	{
		menuPasteAll();
	}
...
void renderMenu()
{
	...
	if (ImGui::BeginMenuBar())
	{
		...
		{
			if (ImGui::MenuItem("New Wavetable", ImGui::GetIO().ConfigMacOSXBehaviors ? "Cmd+N" : "Ctrl+N"))
				menuNewBank();
			if (ImGui::MenuItem("Open Wavetable...", ImGui::GetIO().ConfigMacOSXBehaviors ? "Cmd+O" : "Ctrl+O"))
				menuOpenBank();
			if (ImGui::MenuItem("Save Wavetable", ImGui::GetIO().ConfigMacOSXBehaviors ? "Cmd+S" : "Ctrl+S"))
				menuSaveBank();
			if (ImGui::MenuItem("Save Wavetable As...", ImGui::GetIO().ConfigMacOSXBehaviors ? "Cmd+Shift+S" : "Ctrl+Shift+S"))
				menuSaveBankAs();
...
void renderPreview()
{
	...
	else
	{
		...
		ImGui::SameLine();
		ImGui::SliderFloat("##Morph Z Speed", &morphZSpeed, 0.0f, 10.0f, "Morph Z Speed: %.3f Hz", ImGuiSliderFlags_Logarithmic);
	}
...
void waterfallPage()
{
	ImGui::BeginChild("3D View", ImVec2(0, 0), true);
	{
		...
		ImGui::SliderFloat("##amplitude", &amplitude, 0.01f, 1.0f, "Scale: %.3f");
...
static void refreshStyle()
{
	...
	else if (styleId == 1)
	{
		...
		style.Colors[ImGuiCol_PopupBg] = base[0x4];
	...
	else if (styleId == 2)
	{
		...
		style.Colors[ImGuiCol_PopupBg] = base[0x6];
...
void uiInit()
{
	...
	// Load fonts
	ImGui::GetIO().Fonts->AddFontFromFileTTF("Lekton-Regular.ttf", 15.0);
